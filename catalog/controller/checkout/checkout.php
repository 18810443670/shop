    ...
    public function update()
    {
        ...
        if ($code = $this->getPostValue('payment_method')) {
            if (!in_array($code, ['gateway', 'quick'])) {
                $error = $this->language->get('error_payment_unavailable');
...
//        $data['content_bottom'] = $this->load->controller('common/content_bottom');
    ...
    private function renderAgreeSection()
    {
        ...
        // Register
        if (!$this->isLogged() && $this->config->get('config_account_id')) {
            $this->load->model('catalog/information');
            $information_info = $this->model_catalog_information->getInformation($this->config->get('config_account_id'));
            if ($information_info) {
                $data['text_register'] = sprintf($this->language->get('text_agree'), $this->url->link('information/information/agree', 'information_id=' . $this->config->get('config_account_id')), $information_info['title'], $information_info['title']);
            } else {
                $data['text_register'] = '';
            }
        } else {
